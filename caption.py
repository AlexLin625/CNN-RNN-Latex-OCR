...
def caption_image_beam_search(encoder, decoder, image_path, word_map, beam_size=5):
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            alpha = alpha.view(
                -1, enc_image_size[0], enc_image_size[1]
            ...
            h = decoder.decode_step(
                torch.cat([embeddings, awe], dim=1), h
            )  # (s, decoder_dim)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--img",
        "-i",
        default="./data/CROHME/images/images_train/TrainData2_15_sub_33.png",
        help="path to image",
    )
    parser.add_argument(
        "--model", "-m", default="BEST_checkpoint_CROHME.pth.tar", help="path to model"
    )
    parser.add_argument(
        "--word_map",
        "-wm",
        default="./data/CROHME/vocab.json",
        help="path to word map JSON",
    )
    parser.add_argument(
        "--beam_size", "-b", default=3, type=int, help="beam size for beam search"
    )
    parser.add_argument(
        "--dont_smooth",
        dest="smooth",
