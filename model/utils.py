...
def collate_fn(batch_images):
    ...
    ), torch.zeros((len(proper_items), 1, max_height, max_width))
    labels, labels_masks = torch.zeros(
        (len(proper_items), max_length)
    ).long(), torch.zeros((len(proper_items), max_length))
...
def collate_fn_MyDataset(batch: list[tuple]):
    max_width, max_height, max_length = 0, 0, 0
    ...
    for sample in batch:  # iterate over each samples
        ...
        if (
            image.shape[0] * max_width > 1600 * 320
            or image.shape[1] * max_height > 1600 * 320
        ):
            continue
