...
def label_transform(
    text, start_type="<start>", end_type="<end>", pad_type="<pad>", max_len=160
):
    text = text.split()
...
class MyDataset(Dataset):
    def __init__(
        ...
        self.npy_files_list = npy_files_list[
            int(split_tuple[0] * n_files) : int(split_tuple[1] * n_files)
        ]
        self.n_files = len(self.npy_files_list)
        print(
            f"files {self.npy_files_list} are used as {'train' if is_train else 'val'} set"
        )
        ...
        for file_name in self.npy_files_list:
            temp_images_and_labels: list[dict] = np.load(
                Path(dataset_dir) / file_name, allow_pickle=True
            )
            self.n_samples_list.append(
                self.n_samples_list[-1] + len(temp_images_and_labels)
            )
        ...
        self.images_and_labels: list[dict] = np.load(
            Path(dataset_dir) / self.npy_files_list[0], allow_pickle=True
        )
    ...
    def __getitem__(self, idx):
        ...
        if idx >= self.n_samples_list[self.current_file_idx + 1]:
            self.images_and_labels = np.load(
                Path(self.dataset_dir) / self.npy_files_list[self.current_file_idx + 1],
                allow_pickle=True,
            )
        ...
        image = zoom(
            image, 1 / self.ratio, order=1
        )  # order=1 corresponds to bilinear interpolation
        ...
        if idx == self.n_samples_list[-1] - 1:
            self.current_file_idx = 0
            self.images_and_labels = np.load(
                Path(self.dataset_dir) / self.npy_files_list[0], allow_pickle=True
            )
...
class formuladataset(object):
    ...
    def __init__(
        ...
        self.buckets_index = np.array(
            [i for i in range(len(self.buckets))], dtype=np.int32
        )  # 尺寸索引,用于shuffle
        self.bucket_data = [
            [] for i in range(len(self.buckets))
        ]  # 用于存放不同尺寸的data
        self.img_list = np.array(
            [i for i in self.data.keys()]
        )  # 得到所有的图像名字的列表
    ...
    def _iter(self):
        for size_idx, i in enumerate(self.bucket_data):
            img_batch, cap_batch, cap_len_batch = (
                [],
                [],
                torch.zeros((self.batch_size)).int(),
            )
            idx = 0
            for j in i:
                item = self.data[j]
                ...
                if self.img_transform is not None:
                    img = self.img_transform(
                        img, size=self.buckets[size_idx], ratio=self.ratio
                    )
                ...
                if idx % self.batch_size == 0:
                    ...
                    for ii in range(len(cap_batch)):
                        cap_batch[ii] += [vocab["<pad>"]] * (
                            int(max(cap_len_batch)) - len(cap_batch[ii])
                        )
